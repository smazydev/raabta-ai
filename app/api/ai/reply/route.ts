  ...
  try {
    ...
    if (!ai.autoReply) {
      return NextResponse.json(
        { error: "AI replies are disabled for this tenant (Settings → AI automation)." },
        { status: 403 }
      );
    }
    ...
    const { reply: text, usage } = await generateConversationReply({
      customerName: String(body.customerName ?? "Customer"),
      messages: body.messages ?? [],
      kbContext: String(body.kbContext ?? ""),
      language: parseChatLanguage(body.language),
      romanUrdu: Boolean(body.romanUrdu),
      agentName: body.agentName != null ? String(body.agentName) : undefined,
      agentInstructions: body.agentInstructions != null ? String(body.agentInstructions) : undefined,
      model,
    });
    await chargeAfterChatCompletion(session.tenantId, usage, "openai.api.reply", {}, model);
